...
MCP 和 skill 的柱状条衡量的是各服务器或 skill *活跃期间*花掉的 token，而不是触发它的
那次调用的大小 —— tooltip 两者都会显示。
...
- KPI 卡片 —— 成本、以速率限制权重单位表示的**套餐用量**、输出、缓存读取、LLM 调用数，以及**上下文占用**（最近一次 / 峰值）—— 外加累计花费曲线和按模型的表格
